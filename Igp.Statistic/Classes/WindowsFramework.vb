Public Class WindowsFramework
    Inherits IgpManager.BACK.FrameworkBase


    Private mMsgPopup As MsgPopup
    Public Sub New()
        MyBase.New(Igp.POSResources.SysModule.IgpManager)
    End Sub
    ...
    Public Overrides Function AskForAuthorisation(ByVal nAuthLevelRequested As Integer) As Boolean
        Dim sec As Igp.AccessController.SecurityManager = New Igp.AccessController.SecurityManager(mConn)
        ...
        If result = DialogResult.OK Then
            If sec.HasAuthorisationBy(mUser.Id, nAuthLevelRequested, auth.txtUser.Text, auth.txtPassword.Text) = "OK" Then
                mAudit.AddEntry(Igp.AccessController.AuditLogWriter.EventType.Authorisation, mUser.Id, mTerminalId, auth.txtUser.Text)
                Return True
            Else
                Return False
            End If
