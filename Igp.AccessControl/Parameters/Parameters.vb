...
#Region " Instance variables "
    ...
    'Private oParams As New Hashtable
...
#Region " Methods "
    ...
    Public Function GetColor(ByVal sParameterId As String) As Drawing.Color
        Dim val As Object = GetValue(sParameterId)
        Dim s As String

        Try
            s = DirectCast(val, String)
        Catch ex As Exception
            Throw New ArgumentException(String.Format("Parameter '{0}' couldn't be parsed as a color.", sParameterId))
        End Try

        Dim colors As String() = s.Split(","c)
        If colors.Length <> 3 Then
            Throw New ArgumentException(String.Format("Parameter '{0}' couldn't be parsed as a color. Use 'R,G,B' format.", sParameterId))
        End If
        ...
        Try
            r = CheckColorArgument(colors(0))
            g = CheckColorArgument(colors(1))
            b = CheckColorArgument(colors(2))
        Catch ex As Exception
            Throw New ArgumentException(String.Format("Parameter '{0}' couldn't be parsed as color. Each color component must be from 0 to 255.", sParameterId))
        End Try
